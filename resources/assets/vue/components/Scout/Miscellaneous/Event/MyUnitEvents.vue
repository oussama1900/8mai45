...
<script>
    export default {
        ...
        methods:{
            ...
            getAllEvents(){
                var vm = this;
                axios.get('/api/getMyUnitEvents').then(function (response) {

                    vm.AllEvents = response.data.allevents[0];
                    vm.user = response.data.allevents[1];
                });
