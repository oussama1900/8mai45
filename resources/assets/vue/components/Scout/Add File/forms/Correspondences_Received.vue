                    ...
                    <div class="card-footer">
                           ...
                           <p style="text-align: right;margin-top:0px;margin-bottom:0px">
                               ...
                               <a  class="btn btn-primary btn-circle btn-lg" style="padding-left: 6px; padding-top: 6px;padding-right:8px" @click="remove_document(correspondence,'notapproved')">
                                   <span class="glyphicon glyphicon-remove" aria-hidden="true"  style="font-size:2rem"></span>
                               </a>
...
<div v-if="Correspondences_Not_Appr.length===0">
    <h3 class="label_title" > لا يوجد وثائق </h3>
        ...
        <sweet-modal ref="Authenticate" icon="warning">
            <h3>هل أنت متأكد من أنك تريد وضع ختمك على هذه الوثيقة</h3>
                ...
                <div class="col-md-4 card text-center report-card " style="margin-left:15px;padding-right:0px;padding-left:0px;height:165px" v-for="correspondence_appr in Correspondences_Appr" :id="'correspondence_appr'+correspondence_appr.outing_mail_id">
                    ...
                    <div class="card-footer">
                        ...
                        <p style="text-align: right;margin-top:0px;margin-bottom:0px">
                            ...
                            <a class="btn btn-primary btn-circle btn-lg" style="padding-left: 6px; padding-top: 6px;padding-right:8px" @click="remove_document(correspondence_appr,'approved')">
                                <span class="glyphicon glyphicon-remove" aria-hidden="true" style="font-size:2rem"></span>
                            </a>
...
<script>
    ...
    export default {
        ...
        created:function () {
            ...
            var vm  =this;
            axios.get('/api/GetCorrespondences').then(function (response) {
        ...
        methods:{
            ...
            Authenticate(correspondence){
                var vm = this;
                this.$refs.Authenticate.open();
                $("#confirmation_button").unbind().click(function () {
                    vm.$refs.Authenticate.close();
                    vm.show =true;
                    axios.get("/api/Authenticate_Correspondence/"+correspondence.outing_mail_id).then(function (response) {
                                      console.log(response);
                        var position = vm.Correspondences_Not_Appr.indexOf(correspondence);
                        vm.Correspondences_Not_Appr.splice(position,1);
                        vm.Correspondences_Appr.push(correspondence);
                        vm.show =false;
                    })
            ...
            remove_document(correspondence,type){
              var vm = this;
                this.$refs.delete_warning.open();
                $("#delete_confirmation_button").unbind().click(function () {

                    vm.$refs.delete_warning.close();
                    vm.show =true;
                    axios.delete("/api/delete_document/"+correspondence.outing_mail_id).then(function (response) {
                        if(type.localeCompare('approved')===0){
                            var position = vm.Correspondences_Appr.indexOf(correspondence);
                            vm.Correspondences_Appr.splice(position,1);
                ...
                $("#delete_cancel_button").unbind().click(function () {
